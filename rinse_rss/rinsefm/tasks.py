...
def scrape_podcast_episodes():
    scrape_url = 'http://rinse.fm/podcasts/'
    logging.info("Fetching podcasts from {0}".format(scrape_url))
    ...
    for div in podcasts_page.xpath('//div[contains(@class, "podcast-list-item")]'):
        ...
        content_headers = http_session.head(audio_url).headers
        ...
        episode, created = PodcastEpisode.objects.update_or_create(
            ...
            defaults=dict(
                ...
                audio_url=audio_url,
                audio_content_length=content_headers['Content-Length'],
                audio_content_type=content_headers['Content-Type'],
            )
        )
